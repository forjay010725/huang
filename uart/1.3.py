...
class BlePairer:
    ...
    def enable_broadcast(self, ser):
        ...
        # responses = [
        #     self.send_at_command(ser, "AT+ADV=1,1,200"),  # 开启可连接广播
        #     #self.send_at_command(ser, "AT+ADV?")  # 验证广播状态
        # ]
    ...
    #     """主机扫描从机设备"""
    #     # 启动扫描
    #     self.send_at_command(ser, "AT+SCAN=1,10,1")
    #
    #     scan_data = ""
    #     start_time = time.time()
    #     print(f"[{ser.port}] 开始扫描BLE设备...")
    #
    #     # 收集扫描结果（12秒超时）
    ...
    #             scan_data += data
    #             # 检测扫描结束标记
    #             if "SCAN=1,10,1" in scan_data:
    #                 break
    #         time.sleep(0.5)
    #
    #     print(f"[{ser.port}] 扫描结果:\n{scan_data}")
    #     # 提取MAC地址
    #     return re.findall(
    #         r"([0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2})", scan_data)
    ...
    def auto_pair(self):
        ...
        try:
            ...
            if (
                ...
                print(f"[{self.ser1.port}] 开始扫描...")
                scanned_macs = self.scan_devices(self.ser1)
                print(f"扫描到的设备: {scanned_macs}")

                if mac2 in scanned_macs:
                    print(f"找到目标设备 {mac2}")
                    if self.connect_device(self.ser1, mac2):
                        return True
                else:
                    print(f"未找到设备 {mac2}")

            # 第二次尝试：设备2作为主机，设备1作为从机
            print("\n=== 尝试2: 设备2作为主机 ===")
            if (
                ...
                print(f"[{self.ser2.port}] 开始扫描...")
                scanned_macs = self.scan_devices(self.ser2)
                print(f"扫描到的设备: {scanned_macs}")

                if mac1 in scanned_macs:
                    print(f"找到目标设备 {mac1}")
                    if self.connect_device(self.ser2, mac1):
                        return True
                else:
                    print(f"未找到设备 {mac1}")

            raise Exception("所有配对尝试均失败")
...
if __name__ == "__main__":
    ...
    # 2. 让用户选择两个串口
    try:
        port1 = int(input("请输入第一个设备序号 (1,2,3...): ").strip())
        port2 = int(input("请输入第二个设备序号 (1,2,3...): ").strip())
    except ValueError:
        print("错误：请输入数字序号")
        exit(1)
