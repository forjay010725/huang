...
class BlePairer:
    ...
    def set_role(self, ser, role, slave_mac=None):
        """设置设备角色（主/从）并配置主机自动重连"""
        ...
        if role == 1:
            ...
            if slave_mac:
                print(f"🔗 配置主机自动重连从机: {slave_mac}")
                # 设置自动连接参数（1: 自动连接模式）
                auto_cmd = f"AT+AUTO_CNT=1,{slave_mac}"
                auto_response = ser.send_at_command(auto_cmd, expected_response="+OK")

                if "+OK" not in auto_response:
                    print(f"⚠️ 自动重连配置失败，错误响应: {auto_response}")
                    return False
    ...
    def auto_pair(self):
        ...
        try:
            ...
            # 设备1作为主机
            # 在auto_pair方法中
            if not self.set_role(self.ser1, 1, slave_mac=mac2):  # 增加slave_mac参数
                print("❌ 设备1设置为主机失败")
                return False
...
if __name__ == "__main__":
    ...
    if pairer.auto_pair():
        print("\n🎉 配对成功! 设备已连接并进入透传模式")
        input("___________任意键退出________")
